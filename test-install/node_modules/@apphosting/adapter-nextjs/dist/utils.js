...
export async function readRoutesManifest(distDir) {
 ...
 */
export function populateOutputBundleOptions(rootDir, appDir) {
    ...
    const outputDirectoryAppPath = join(outputBundleDir, process.env.MONOREPO_COMMAND ? relative(rootDir, appDir) : "");
    return {
...
// Run build command
export function build(cwd, cmd = DEFAULT_COMMAND, ...argv) {
    // Set standalone mode
 ...
 */
export async function generateOutputDirectory(rootDir, appDir, outputBundleOptions, nextBuildDirectory) {
...
async function moveResources(appDir, outputBundleAppDir) {
...
async function generateBundleYaml(outputBundleOptions, nextBuildDirectory, cwd) {
    ...
    await writeFile(outputBundleOptions.bundleYamlPath, yamlStringify({
        headers,
        redirects,
        rewrites,
...
export async function validateOutputDirectory(outputBundleOptions) {
    if (!(await fsExtra.exists(outputBundleOptions.outputDirectoryBasePath)) ||
